...
const DEFAULT_URL =
  'http://localhost:8890/license/assistance/Typofonderie-AiglonPro';

const TARGET_URL = String(__ENV.TARGET_URL || DEFAULT_URL);
const EXPECT_STATUS = toInt(__ENV.EXPECT_STATUS, 200);

const AVG_RPS = toInt(__ENV.AVG_RPS, 50);
const DURATION = String(__ENV.DURATION || '20m');
const PREALLOCATED_VUS = toInt(__ENV.PRE_ALLOCATED_VUS, 30);
const MAX_VUS = toInt(__ENV.MAX_VUS, 200);
